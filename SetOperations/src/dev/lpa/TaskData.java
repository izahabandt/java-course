...
public class TaskData {
    ...
    public static Set<dev.lpa.Task> getTasks(String owner) {

        Set<dev.lpa.Task> taskList = new HashSet<>();
        ...
        for (String taskData : selectedList.split("\n")) {
            ...
            dev.lpa.Status status = (data.length <= 3) ? dev.lpa.Status.IN_QUEUE :
                    dev.lpa.Status.valueOf(data[3].toUpperCase()
                            .replace(' ', '_'));

            dev.lpa.Priority priority = dev.lpa.Priority.valueOf(data[2].toUpperCase());
            taskList.add(new dev.lpa.Task(data[0], data[1], user,
                    priority, status));
        }
